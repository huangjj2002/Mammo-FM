...
def create_folds(csv_path, label_col="cancer", n_folds=5, seed=42, output_path=None,
    ...
    split_values = set(df["split"].dropna().unique())
    allowed_splits = {"training", "test"}
    bad_splits = sorted(split_values - allowed_splits)
    if bad_splits:
        raise ValueError(f"Unsupported split value(s): {bad_splits}. Expected only {sorted(allowed_splits)}.")
    ...
    for fold in range(args.n_folds):
        ...
        try:
            for epoch in range(args.epochs):
                ...
                logger.add_scalar("valid/mean_uncertainty", mean_uncertainty, epoch + 1)
